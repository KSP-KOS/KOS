...
- [ ] Create a ZIP file that has only the `GameData` folder of the `\Resources\` directory in it.  Make sure the ZIP's folder structure starts with `GameData`.  (In other words, the filename of kOS.version in the ZIP would be `GameData\kOS\kOS.version` and NOT `\kOS\kOS.version`.)
