...
namespace kOS.Safe.Persistence
{
    public abstract class Volume : Structure
    {
        ...
        public void Assemble(StringValue assembly, StringValue filePath)
        {
            var parts = AssemblyObject.Assemble(assembly, null).ToList();
            SaveObjectFile(filePath, parts);
        }

        public StringValue Disassemble(StringValue filePath)
        {
            ...
            return new StringValue(AssemblyObject.Disassemble(parts));
        }
        ...
        private void InitializeVolumeSuffixes()
        {
            ...
            AddSuffix("DISASSEMBLE" , new OneArgsSuffix<StringValue, StringValue>((filename) => Disassemble(filename)));
            AddSuffix("ASSEMBLE", new TwoArgsSuffix<StringValue, StringValue>((assembly, filename) => Assemble(assembly, filename)));
        }
